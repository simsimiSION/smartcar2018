...
def get_path_optim(origin, queen):
    ...
    que_opt = dp(queen)
    chess_opt = []
    for o, q in zip(origin, queen):
        if o == q:
            chess_opt.append(get_num(o))
            que_opt.remove(q)
            ori_opt.remove(o)


    length_limit = len(que_opt)
    if length_limit == 0:
        print(' 步数太少了。 自己动吧')
        return [],[], 9999
    elif length_limit == 1:
        ...
        optim_path.append(get_num(ori_opt[0]))
        optim_path.append(get_num(que_opt[0]))

        best_dis =  get_dis(que_opt[0], ori_opt[0])
    else:
        ...
        best_dis = 0
        index_list = []
        index = que_dict[str(ori_opt[0])]
        index_list.append(index)
        ori_opt.remove(ori[index])

        # 获得次序
        while len(index_list) != length_limit:
            temp, dis = getLeast(queen[index], ori_opt)
            best_dis += dis
            index = que_dict[str(temp)]
            index_list.append(index)
            if ori != []:
                ori_opt.remove(origin[index])
...
def mulEssentialFunc(i, dis_num, ori_list, solution):
    ori_essential = []
    que_essential = []
    ...
    dis_num[i].dis = min_distance
    dis_num[i].path = path
    dis_num[i].chess = chess
    dis_num[i].ori = ori_essential
    dis_num[i].que = que_essential
...
def dis_num_get_least(dis_dict):
    best_path = []
    best_chess = []
    best_ori = []
    best_que = []
    ...
    for key in dis_dict.keys():
        if dis_dict[key].dis < best_distance:
            best_distance = dis_dict[key].dis
            best_path = dis_dict[key].path
            best_chess = dis_dict[key].chess
            best_ori = dis_dict[key].ori
            best_que = dis_dict[key].que


    for _ in range(2):
        print()
    chess, path = outputFormat(best_distance, best_ori, best_que)
    print("转花前的效果")
    queenprint2(best_ori)
    print("转化后的效果")
    queenprint2(best_que)

    print('\n最优路径为: ' + str(best_path))
    print('最优棋子为: ' + str(best_chess))
    print('最短距离为: ' + str(best_distance))
    return best_path, best_chess
...
def totalFunc(chess):
    # 获得可行的八皇后
    queen_list_essential = list(queens(8))
    length = len(queen_list_essential)

    global_dict = {}
    for i in range(length):
        global_dict[i] = dis_number()

    # 多进程执行 =======================================
    jobs = []
    p = multiprocessing.Pool()
    for i, temp in enumerate(queen_list_essential):
        jobs.append(p.apply_async(mulEssentialFunc, args=(i, global_dict, inputFormat(chess), temp)))
    ...
    for i, job in enumerate(jobs):
        global_dict[i] = job.get()

    # =================================================

    return dis_num_get_least(global_dict)


if __name__ == '__main__':

    chess = [6, 10, 23, 25, 36, 48, 53, 59]

    print(totalFunc(chess))

    # ori = [[0, 5], [1, 1], [2, 6], [3, 0], [4, 3], [5, 7], [6, 4], [7, 2]]
    # que = [[0, 6], [1, 2], [2, 5], [3, 1], [4, 3], [5, 7], [6, 4], [7, 2]]
    # print(get_path_optim(ori, que))
    #
    #
