...
import sys, random
import numpy as np
from put_single.least_dis import get_path_input
from put_single2.queen import essentialFunc, inputFormat
from put_single4.queen import totalFunc
...
from handle import *
from go import *
from go_single import queue_move_single, get_chess_list
...
#32|14-right|22-right|32
#32|15-left|23-left|32
#62|13-up|14-up|6
...
class Map_gui(QWidget):
    ...
    def init_ui(self):
        ...
        self.combox.addItem('queen_single')
        ...
        # button change chess
        self.button_add_chess = QPushButton('opposite', self)
        self.button_add_chess.move(850,  210)
        self.button_add_chess.resize(30, 30)
        self.button_add_chess.clicked.connect(self.opposite_chess)

        # button add chess
        self.button_add_chess = QPushButton('加棋子', self)
        self.button_add_chess.move(880,  210)
        self.button_add_chess.resize(25, 30)
        self.button_add_chess.clicked.connect(self.add_chess)

        # button delete chess
        self.button_del_chess = QPushButton('删棋子', self)
        self.button_del_chess.move(905,  210)
        self.button_del_chess.resize(25, 30)
    ...
    def on_click(self):
        ...
        if self.system_mode == QUEEN:
            ...
            if self.press == False:
                ...
                try:
                    textboxValue = textboxValue.split('|')
                    length = len(textboxValue)
                    for index,text in enumerate(textboxValue):
                    ...
                    for n, d in zip(self.dis_list[1:-1], self.barrier_list):
                        self.to_boundary_list(n, d)
                    self.button_status = DOWN
                except:
                    reply = QMessageBox.information(self, "警告", "格式错误", QMessageBox.Yes | QMessageBox.No)
            else:
                data = self.graph.data_to_num()
                from_place = data[0]
                to_place = data[-1]

                self.dis_list = [from_place]
                for temp in data[1:-1]:
                    d, n = self.graph.format_barrier(from_place, temp)
                    self.dis_list.append(n)
                    self.barrier_list.append(d)
                self.dis_list.append(to_place)
            ...
            if self.press == False:
                ...
                self.dis_list = self.graph.data_to_num()
                if self.dis_list[1] not in self.chess_list:
                    self.chess_list.extend(get_chess_list(self.dis_list))
                self.button_status = DOWN
        elif self.system_mode == QUEEN_SINGE:
            if self.press == False:
            #===============================================================================================
                self.dis_list = [61, 45, 47, 37, 24, 29, 30, 28, 13, 27, 50, 44, 60, 36, 1, 1]
                self.chess_list = [35]
            #=================================================================================================
                self.chess_list.extend(get_chess_list(self.dis_list))
                self.button_status = DOWN
            else:
                #self.dis_list, chess_opt = get_path_input(self.graph.data_to_num())
                #self.dis_list, chess_opt = essentialFunc(inputFormat(self.graph.data_to_num()))
                self.dis_list, chess_opt = totalFunc(self.graph.data_to_num())
                self.chess_list = get_chess_list(self.dis_list)
                self.chess_list.extend(chess_opt)
                reply = QMessageBox.information(self,
                                                "警告",
                                                "the start place:" + str(self.dis_list[0]),
                                                QMessageBox.Yes | QMessageBox.No)
                if reply == QMessageBox.Yes:
                    self.button_status = DOWN

            if self.dis_list[0] == self.dis_list[1]:
                self.queen_single_flag_in = 1
            if self.dis_list[-1] == self.dis_list[-2]:
                self.queen_single_flag_out = 1
    ...
    def opposite_chess(self):
        if self.press:
            self.input_chess_show = 1
            temp = self.graph.data_to_num()
            if len(temp) > 1:
                reply = QMessageBox.information(self, "警告", "格式错误", QMessageBox.Yes | QMessageBox.No)
                return

            if self.opposite_ch in self.chess_list:
                self.chess_list.remove(self.opposite_ch)
            self.opposite_ch = temp[0]
            self.chess_list.append(self.opposite_ch)
            self.input_chess_show = 0
    ...
    def to_boundary_list(self, num, dir):
        ...
        if dir == 'up' or dir == 'down':
            ...
            if dir == 'right':
                bias = 1
            barrier = [type, position[1]+bias, max(position[0], 0)]
    ...
    def combox_click(self, text):
        ...
        elif text == 'queen_single':
            self.system_mode = QUEEN_SINGE
    ...
    def interrupt(self):
        ...
        #      if move_in_status:
        #          if self.queen_single_flag_in == 0:
        #              if self.dis_list[0] in self.chess_list:
        #                  self.queen_single_flag = 1
        ...
        #                 self.barrier_out_flag, self.queen_single_flag, self.chess_list,self.status, self.button_status, self.path, self.optim, self.mode, direct, self.bd, self.barrier_dir, self.barrier_flag, self.last_position = barrier(
        #                     self.barrier_out_flag, self.queen_single_flag, self.chess_list,number, location, direct, img_show, img_origin, self.status, self.button_status, self.dis, self.queen_mode, self.mode, self.last_angel, self.path, self.optim, to_map, self.bd, self.barrier_dir, self.barrier_flag, self.last_position, self.dis_list)
        #
        #             else:
        #                 self.queen_single_flag, self.chess_list,self.status, self.button_status, self.path, self.optim, self.mode, direct, self.last_position = queue_move_single(self.queen_single_flag_out, self.queen_single_flag, self.chess_list, number, location, direct, img_show, img_origin, self.status, self.button_status, self.dis,self.queen_mode, self.mode, self.last_angel, self.path, self.optim, to_map, self.last_position)
        #
        #             if self.queen_single_flag > 0:
        #                 _, circle_list, is_circle = detect_circle_2(img_show)
        #                 if is_circle == 0:
        #                     self.queen_single_flag += 1
        #                 if self.queen_single_flag == 2:
        #                     self.queen_single_flag = 0
        #         elif self.system_mode == CHESS_MOVE:
        #             self.queen_single_flag, self.chess_list,self.status, self.button_status, self.path, self.optim, self.mode, direct, self.last_position = queue_move_single(self.queen_single_flag_out, self.queen_single_flag, self.chess_list, number, location, direct, img_show, img_origin, self.status, self.button_status, self.dis,self.queen_mode, self.mode, self.last_angel, self.path, self.optim, to_map, self.last_position)
        #
        ...
        #     time_out, self.status, self.button_status, self.path, self.optim, self.mode, direct, self.last_position = move_out(
        #          number, location, direct, img_show, img_origin, self.status, self.button_status, self.dis, self.queen_mode, self.mode, self.last_angel, self.path, self.optim, to_map, self.last_position)
        #     self.main_status_count += 1
        #     if self.main_status_count == time_out:
        #         self.main_status_count = 0
        #         self.main_status = MAIN_WAIT
        ...
        #         if len(self.dis_list) > 2:
        #             self.dis_list = self.dis_list[1:]
        #             self.barrier_list = self.barrier_list[1:]
        #             self.status = WAIT
        #
        # elif self.system_mode == CHESS_MOVE:
        #     if self.status == FINISH:
        ...
        #             self.status = WAIT
        #
        #         if len(self.dis_list)%2 == 0 and self.queen_mode != 3:
        #             self.queen_mode = PUT
        #         elif len(self.dis_list)%2 == 1 and self.queen_mode != 3:
        #             self.queen_mode = TAKE
        #
        #     elif self.status == RUNNING_GO:
        #         self.queen_mode = 3
        ...
        # #     elif self.main_status == MAIN_RUN:
        # #         if self.status == WAIT or (self.status == PRE_BACKING and self.system_mode != BARRIER) or self.arm_count != 0:
        # #             self.uart.send_all([0.0, 0.0, 0.0], self.arm_status)
        # #
        # #         if direct != [] and (self.status == RUNNING or self.status == ARMING or (self.status == PRE_BACKING and self.system_mode == BARRIER)) and self.arm_count == 0:
        # #             self.uart.send_all(direct, self.arm_status)
        # #
        # #     elif (self.main_status == MAIN_IN or self.main_status == MAIN_OUT) and direct !=[]:
        # #             self.uart.send_all(direct, self.arm_status)
        # #
        # # else:
    ...
    def paintEvent(self, e):
        ...
        if self.press:
            if self.button_status != DOWN or self.input_chess_show == 1:
                self.graph.draw(qp)
        self.drawChess(qp)
        qp.end()
